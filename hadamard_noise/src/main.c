...
// Usage:
// $ hadamard transform qubits_num accuracy input_filename output_filename [-t]
// $ hadamard generate qubits_num output_filename [-t]
...
int main(int argc, char *argv[])
{
    ...
    omp_set_num_threads((int) args.threads_num);
...
static int8_t main_transform(struct args *args, int32_t my_rank, int32_t comm_size)
{
    ...
    err = quant_state_vector_read_file(&v, fh_in);
    if (err) {
        main_print(my_rank, "Vector read failed.\n");
        return err;
    }
    main_print(my_rank, "Vector read is done.\n");

    struct quant_state_vector w_noise = {0};
    quant_state_vector_alloc(&w_noise, args->qubits_num);

    main_print(my_rank, "Transformation with noise started...\n");
    MPI_Barrier(MPI_COMM_WORLD);
    double start = MPI_Wtime();

    err = quant_n_hadamard_transform_noise(args, &v, &w_noise, args->accuracy);
    if (err) {
        main_print(my_rank, "Transformation with noise failed.\n");
    }
    ...
    main_print(my_rank, "Time elapsed with noise:\n%lf\n", elapsed_max);

    err = quant_state_vector_print_file(&w_noise, fh_out);
    if (err) {
        main_print(my_rank, "Can't write the w_noise vector to the file\n");
        return err;
    }
    ...
    if (args->loss) {
        err = main_loss(args, &v, &w_noise);
    }

    if (args->test) {
        main_print_test_info(args, &v, &w_noise);
    }

    quant_state_vector_free(&v);
    quant_state_vector_free(&w_noise);
        ...
        struct quant_state_vector *w_noise)
{
    ...
    offset += snprintf(buff + offset, buff_size - offset, "\nState vector w_noise:\n");
    offset += quant_state_vector_snprint_info(buff + offset, buff_size - offset, w_noise);
        ...
        struct quant_state_vector *w_noise)
{
    ...
    // Accuracy 0 for no noise.
    int8_t err = quant_n_hadamard_transform_noise(args, v, &w, 0);
    if (err) {
        main_print(my_rank, "Transformation withouth noise failed.\n");
    }

    double loss = quant_precision_loss(&w, w_noise);
    if (loss < 0) {
        main_print(my_rank, "Precision loss calculation failed.\n");
        err = 1;
    }
    main_print(my_rank, "Precision loss:\n%lf\n", loss);
